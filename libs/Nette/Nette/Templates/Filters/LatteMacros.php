...
?>' . $s;
	...
	{
		return '$iterator = $_l->its[] = new NSmartCachingIterator(' . preg_replace('# +as +#i', ') as ', $this->formatMacroArgs($content), 1);
	}
	...
	{
		if ($matches = NString::match($s, '#^((?>'.NLatteFilter::RE_STRING.'|[^\'"\s,]+)+)\s*,?\s*(.*)$#')) { // token [,] tail
			$s = $matches[2];
			return $matches[1];
		}
		return NULL;
